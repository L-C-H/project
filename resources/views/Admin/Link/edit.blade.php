...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" action="/adminlink/{{$data->id}}" method="post">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text"  placeholder="" id="author" name="web" value="{{$data->web}}">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">公司：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text"  placeholder="" id="author" name="company" value="{{$data->company}}">
			</div>
		</div>
		{{csrf_field()}}
		{{method_field('PUT')}}
		<input type="hidden" name="changetime" value="{{time()}}">
		<div class="row cl">
