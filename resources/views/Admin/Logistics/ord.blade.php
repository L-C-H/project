		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<tr class="text-c">
					...
					<td>{{$data->id}}</td>
					<td class="text-l">{{$data->order_id}}</td>
					<td class="text-c">{{$data->name}}</td>
					<td class="td-status">{{$data->goods_name}}</td>
					<td class="td-status"><img src="{{$data->goods_pic}}" width="30"></td>
					<td class="td-status">{{$data->order_num}}</td>
					<td class="td-status">{{$data->goods_price}}</td>
					<td class="td-status">{{$data->price}}</td>
					<td class="td-status">{{$data->created_at}}</td>
					<td class="td-status">{{$data->updated_at}}</td>
...
function picture_stop(obj,id){
	layer.confirm('确认要禁用吗？',function(index){
		$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="picture_start(this,id)" href="javascript:;" title="使用"><i class="Hui-iconfont">&#xe603;</i></a>');
		$(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius">禁用</span>');
		$(obj).remove();
		layer.msg('使用!',{icon: 5,time:1000});
	});
}
...
function picture_start(obj,id){
	layer.confirm('确认要使用吗？',function(index){
		...
		$(obj).parents("tr").find(".td-status").html('<span class="label label-success radius">已使用</span>');
		$(obj).remove();
